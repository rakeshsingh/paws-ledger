...
### Create Dedicated Application User & Directories

```bash
# Create the user
sudo adduser --disabled-password --gecos "" paws
sudo usermod -aG docker paws

# Create application and data directories
sudo mkdir -p /home/paws/paws-ledger/data
sudo chown -R paws:paws /home/paws/paws-ledger
...
5. Go to the **Public Hostname** tab:
    ...
    * **Service Type**: `HTTP`
    * **URL**: `app:8080` (This matches the internal service name in `docker-compose.yml`).

## 3. Upload Files & Configure Environment

Upload your project files to `/home/paws/paws-ledger`. **Ensure the `data/` directory exists and is writable by the container user (UID 1000).**

As the `paws` user, create your `.env` file:
...
# Google OAuth (Must match Google Cloud Console Redirect URIs)
GOOGLE_CLIENT_ID=your_google_id.apps.googleusercontent.com
GOOGLE_CLIENT_SECRET=your_secret_key
GOOGLE_CALLBACK_URL=https://paws.yourdomain.com/api/v1/auth/callback
...
* **Database Permissions**: If the app fails to start due to "Read-only database", ensure the `data/` folder is owned by UID 1000:

    ```bash
    sudo chown -R 1000:1000 /home/paws/paws-ledger/data
    ```
